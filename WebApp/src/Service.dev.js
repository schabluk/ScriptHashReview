 ...
 * Deployed via API.
 */
const contractHash = '8aaea19887722f889a3fa4982ad8f77bef65d4f1'
  ...
  contract: {
            ...
            const print = stack =>
              stack.map(o => {
                if (Array.isArray(o.value)) {
                  ...
                  if (o.type === 'Integer') return parseInt(o.value)
    ...
    async getReviews (address, scriptHash) {
      try {
        ...
        throw new Error(`API getTokens: ${error.message}`)
      }
    },
    async getNumberOfTokens () {
      try {
        ...
        throw new Error(`API getReviewFromAddress: ${error.message}`)
      }
    },
